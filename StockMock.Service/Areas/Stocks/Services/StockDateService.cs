        ...
        : BaseDayService<StockDateService>(context, mapper, cancellationToken, logger, dayService)
    {
        ...
        public async Task AddAsync(StockDateDto dto)
        {
            ...
            if (!dto.ClosingPrice.HasValue || !dto.Gain.HasValue || !dto.PreClosingPrice.HasValue)
            {
                ...
                if (!dto.PreClosingPrice.HasValue)
                {
                    ...
                    else if (dto.ClosingPrice.HasValue && dto.Gain.HasValue)
                        dto.PreClosingPrice = dto.ClosingPrice / (1 + dto.Gain!.Value);
                }
        ...
        public async Task ImportAsync(Stream stream, string fileName)
        {
            ...
            for (int index = 0; index < rows.Count(); index++)
            {
                ...
                if (!row.Gain.HasValue)
                    importSb.AppendErrmsg("涨跌幅未录入");

                if (!row.PreClosingPrice.HasValue)
                    row.PreClosingPrice = row.ClosingPrice / (1 + row.Gain!.Value);
